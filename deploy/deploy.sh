...
FLPE_REPOSITORY=$PREFIX-$NAME-flpe
MOI_REPOSITORY=$PREFIX-$NAME-moi

# Deploy Container Image
cd deploy/
echo "./deploy-ecr.sh $REGISTRY $FLPE_REPOSITORY Dockerfile-flpe $IS_LOCAL"
./deploy-ecr.sh $REGISTRY $FLPE_REPOSITORY Dockerfile-flpe $IS_LOCAL
echo "./deploy-ecr.sh $REGISTRY $MOI_REPOSITORY Dockerfile-moi $IS_LOCAL"
./deploy-ecr.sh $REGISTRY $MOI_REPOSITORY Dockerfile-moi $IS_LOCAL
cd ..

# Deploy Terraform
cd terraform/
terraform init -reconfigure -backend-config="bucket=$S3_STATE" -backend-config="key=$NAME.tfstate" -backend-config="region=us-west-2" -backend-config="profile=$PROFILE"
