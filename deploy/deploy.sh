...
./deploy-ecr.sh $REGISTRY $REPOSITORY-flpe $PREFIX Dockerfile-flpe $PROFILE
./deploy-ecr.sh $REGISTRY $REPOSITORY-moi $PREFIX Dockerfile-moi $PROFILE

# Deploy Terraform
cd terraform/
terraform init -reconfigure -backend-config="bucket=$S3_STATE" -backend-config="key=postdiagnostics.tfstate" -backend-config="region=us-west-2" -backend-config="profile=$PROFILE"
